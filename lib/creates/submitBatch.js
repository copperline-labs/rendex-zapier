...
const batchPdfDynamicFields = (_z, bundle) => {
    const format = bundle.inputData?.format || "png";
    if (format !== "pdf")
        return [];
    return screenshotFields_1.pdfFieldsArray.filter((f) => f.key !== "pdfMargin");
};
const batchFields = [
    {
        key: "urls",
    ...
    },
    {
        ...
        type: "string",
        required: false,
        helpText: "Optional. Want another Zap to run when the entire batch finishes? Create a second Zap, pick 'Webhooks by Zapier → Catch Hook' as its trigger, copy the URL Zapier gives you, and paste it here. Leave empty to poll with 'Get Batch Status' instead.",
    },
    {
        key: "cacheTtl",
...
const perform = async (z, bundle) => {
    ...
    const defaults = (0, request_1.buildRequestBody)(input);
    for (const key of BATCH_STRIPPED_KEYS)
        delete defaults[key];
    const body = { urls, defaults };
    let validatedWebhookUrl;
    try {
        validatedWebhookUrl = (0, request_1.validateOptionalWebhookUrl)(input.webhookUrl, "Webhook URL");
    }
    catch (err) {
...
exports.default = {
    ...
    operation: {
        inputFields: [
            ...batchFields,
            ...screenshotFields_1.advancedFields.filter((f) => BATCH_DEFAULT_KEYS.includes(f.key)),

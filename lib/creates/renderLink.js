...
const expiresInField = {
    ...
    type: "integer",
    required: false,
    helpText: "How long the link stays valid, in seconds (3600–2592000). Leave blank to use the default (30 days).",
};
const perform = async (z, bundle) => {
    const body = (0, request_1.buildRequestBody)(bundle.inputData);
    if (bundle.inputData.expiresIn) {
        body.expiresIn = parseInt(bundle.inputData.expiresIn, 10);
    }
    const response = await z.request({
        method: "POST",
        url: constants_1.RENDER_LINK_ENDPOINT,
        body,
    });
    ...
    return {
        ...
        format: data?.format ?? bundle.inputData.format ?? "png",
...
exports.default = {
    ...
    display: {
        ...
        description: "Create a hosted, auto-caching image or PDF link for a URL, HTML, or Markdown — perfect for og:image social preview tags.",
    },
    operation: {
        inputFields: [

...
const handleErrors = async (response, z, _bundle) => {
    ...
    if (response.status === 429) {
        ...
        throw new z.errors.ThrottledError(withUpgrade("Rate limited by Rendex (the free plan allows 3 requests/minute). Zapier will retry automatically — upgrade for a higher limit."), 60);
    }
    return response;
};

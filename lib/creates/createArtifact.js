...
const perform = async (z, bundle) => {
    ...
    if (input.inputFormat)
        body.inputFormat = input.inputFormat;
    ...
    const branding = {};
    if (input.header)
        branding.header = input.header;
    if (input.accentColor)
        branding.accentColor = input.accentColor;
    if (input.logo)
        branding.logo = (0, request_1.normalizeUrl)(input.logo);
    if (input.footer)
        branding.footer = input.footer;
    if (Object.keys(branding).length > 0)
        body.branding = branding;
    if (input.expiresIn)
        body.expiresIn = parseInt(input.expiresIn, 10);
    ...
    const response = await z.request({
        method: "POST",
        url: constants_1.ARTIFACT_ENDPOINT,
        body,
    });
...
exports.default = {
    ...
    display: {
        ...
        description: "Turn Markdown or HTML into a branded, shareable document in one step — get back a hosted PDF, PNG, and share link. Perfect for reports, invoices, and one-pagers.",
    },
    operation: {
        inputFields: [
            {
                ...
                type: "text",
                required: true,
                helpText: "The Markdown or HTML to turn into a branded document. Headings, lists, links, tables, and code blocks are styled for you. Up to ~4 MB.",
            },
            {
                key: "inputFormat",
            ...
            },
            {
                ...
                required: false,
                helpText: "Which files to produce. Pick PDF, PNG, or both. Each format counts as one render. Leave empty to get both PDF and PNG.",
            },
            {
                key: "header",
                label: "Brand Name",
                type: "string",
                required: false,
                helpText: "Optional brand or document name shown in the header beside your logo (and used as the share page title).",
            },
            {
                key: "logo",
                label: "Logo URL",
            ...
            },
            {
                ...
                type: "string",
                required: false,
                helpText: "Optional accent color for the header bar, links, and headings — a hex value like #EA580C or a CSS color name.",
            },
            {
                key: "footer",
                label: "Footer",
            ...
            },
            {
                ...
                type: "integer",
                required: false,
                helpText: "How long the hosted PDF, PNG, and share links stay valid, in seconds (3600–2592000). Leave blank to use the default (24 hours).",
            },
            {
                key: "data",
                ...
                type: "dict",
                required: false,
                helpText: "Optional. Fill in the {{placeholders}} in your Content before rendering. Add one row per placeholder — the key is the placeholder name (without curly braces) and the value is what replaces it. For example: key = name, value = Alice replaces {{name}} with Alice.",
            },
        ],
        perform,
        sample: {
            pdfUrl: "https://api.rendex.dev/v1/images/captures/usr_abc123/2026-06/11111111-2222-3333-4444-555555555555.pdf?expires=1700000000&sig=a1b2c3d4e5f6",
            pngUrl: "https://api.rendex.dev/v1/images/captures/usr_abc123/2026-06/66666666-7777-8888-9999-000000000000.png?expires=1700000000&sig=a1b2c3d4e5f6",
            shareUrl: "https://api.rendex.dev/v1/images/captures/usr_abc123/2026-06/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee.html?expires=1700000000&sig=a1b2c3d4e5f6",
            expiresAt: "2026-07-23T12:00:00.000Z",
        },
    },
};
//# sourceMappingURL=createArtifact.js.map